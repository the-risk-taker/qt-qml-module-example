...
In this example the [qt_add_qml_module()](https://doc.qt.io/qt-6/qt-add-qml-module.html) is used to make two modules: the module with `main.qml` and the module with `Controls`. Controls is just a umbrella for project controls - here just dummy button. Then those modules are linked into main executable.

Having module with `main.qml` in it requires to load it with `QmlApplicationEngine::loadFromModule()` - see `main.cpp`.
